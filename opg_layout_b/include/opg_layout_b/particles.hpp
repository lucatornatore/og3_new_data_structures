/**
 * @file particles.hpp  (layout B', fine)
 * @brief All particle structs for layout B' — self-contained in this layout.
 *
 * Layout B' ("fine-grained B"): each access-pattern cluster is a separate
 * small struct, and each small struct lives in its own flat array owned by
 * the container. Common-side and type-specific-side arrays are linked by
 * an explicit PLinkage cross-index (one uint32 per common slot).
 *
 * Everything in this file lives in namespace opg::layout_b. There is NO
 * sharing of particle types with layout_c or layout_b_coarse: each layout
...
#ifndef OPG_LAYOUT_B_PARTICLES_HPP
#define OPG_LAYOUT_B_PARTICLES_HPP
...
#endif // OPG_LAYOUT_B_PARTICLES_HPP

...
// =============================================================================
// Fixed-size field array with a zero-width specialisation
// =============================================================================
//
// Used for optional compile-time-width arrays such as metals[NMET]. When N==0
// the storage disappears entirely, which is what we want for configs that
// keep the thermochemistry block but do not store elemental abundances.
...
template<typename T>
struct field_array<T, 0> {};

static_assert(std::is_trivially_copyable_v<field_array<real_t, 3>>);
static_assert(std::is_standard_layout_v<field_array<real_t, 3>>);
static_assert(std::is_trivially_copyable_v<field_array<real_t, 0>>);
static_assert(std::is_standard_layout_v<field_array<real_t, 0>>);

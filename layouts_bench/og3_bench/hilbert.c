...
pkey_t hilbert_key_3d_ijk(uint32_t x, uint32_t y, uint32_t z, unsigned bits)
{
  ...
  if (bits > 21) bits = 21;
  pkey_t ipos[3];
  pkey_t mask_all = (((pkey_t)1) << bits) - 1;
  ipos[0] = ((pkey_t)x) & mask_all;
  ipos[1] = ((pkey_t)y) & mask_all;
  ipos[2] = ((pkey_t)z) & mask_all;
  ...
  //unsigned long long _k = 0;
  // _k |= 5;
  //binary_print(_k);
  //  printf("??\n");

  for (pkey_t mask = ((pkey_t)1) << (bits - 1); mask > 0; mask >>= 1) {
    count++;
    unsigned char pix = ((ipos[0] & mask) ? 4 : 0) | ((ipos[1] & mask) ? 2 : 0) | ((ipos[2] & mask) ? 1 : 0);

    //binary_print(key);
    //  printf(" <= before <<\n");
    key <<= 3;
    //binary_print(key);
    //printf(" <= before | w %d\n",(int)subpix3[rotation][pix]);
